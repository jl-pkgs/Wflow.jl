@testitem "unit: kinematic_wave" begin
    ...
    q, crossarea = RiverRouting.kinematic_wave(q_in, q_prev, q_lat, alpha, dt, dx)
    ...
    q_prev = 0.0
    q_lat = 0.0
    @test RiverRouting.kinematic_wave(q_in, q_prev, q_lat, alpha, dt, dx) == (0.0, 0.0)
end
...
@testitem "unit: ssf_celerity" begin
    ...
    kh_profile = RiverRouting.KhExponential([0.0002795374658372667], [1.8001038115471601])
    @test RiverRouting.ssf_celerity(water_table_depth, slope, theta_e, kh_profile, i) ≈
          3.4838105601686665e-6

    # Case kh_profile::KhExponentialConstant
    kh_profile = RiverRouting.KhExponentialConstant(kh_profile, [0.2])
    @test RiverRouting.ssf_celerity(water_table_depth, slope, theta_e, kh_profile, i) ≈
...
@testitem "unit: kw_ssf_newton_raphson" begin
    ...
    @test RiverRouting.kw_ssf_newton_raphson(ssf, constant_term, celerity, dt, dx) ≈
          0.01090947420564454
end
...
@testitem "unit: accucapacity" begin
    ...
    capacity = fill(1.5, 6)
    flux, new_material =
        RiverRouting.accucapacityflux_state(material, network, capacity, dt)
    @test new_material != material
    @test new_material == dt .* [0.0, 0.5, 0.5, 0.0, 3.5, 1.5]
    @test flux == Float64[0.5, 1.5, 1.5, 1, 1.5, 1.5]
    flux_ = RiverRouting.accucapacityflux(material, network, capacity, dt)
    @test flux == flux_
...
@testitem "unit: kinwave_river_update!" begin
    ...
    river_flow_model = RiverRouting.RiverFlowModel(;
        routing_method = RiverRouting.KinematicWave(),
        timestepping = RiverRouting.TimeStepping(; stable_timesteps = zeros(n)),
        boundary_conditions = RiverRouting.RiverFlowBC(;
            n,
        ...
        parameters = RiverRouting.RiverFlowParameters(;
            flow = RiverRouting.ManningFlowParameters(;
        ...
        variables = RiverRouting.RiverFlowVariables(;
            n,
            q = [0.5499295110293246, 3.0005238507869465],
        ),
        allocation = RiverRouting.NoAllocationRiverModel(n),
        floodplain = nothing,
    )
    graph = DiGraph(2)
    add_edge!(graph, 1, 2)
    domain = TestDomainRiver(;
        network = RiverGraphs.NetworkRiver(;
            graph,
        ...
        parameters = TestRiverParameters(; flow_width, flow_length),
    )
    dt = RiverRouting.stable_timestep(river_flow_model, domain.parameters.flow_length, 0.05)

    RiverRouting.kinwave_river_update!(river_flow_model, domain, dt)
...
@testitem "unit: local inertial river flow with one reservoir" begin
    ...
    n = 3
    river_flow_model = RiverRouting.RiverFlowModel(;
        routing_method = RiverRouting.LocalInertial(),
        ...
        parameters = RiverRouting.RiverFlowStaggeredParameters(;
            n,
            n_edges = 2,
            active_n = [1, 3],
            active_e = [1],
        ...
        variables = RiverRouting.RiverFlowStaggeredVariables(;
            n_cells = n,
            n_edges = 2,
        ...
        floodplain = nothing,
        allocation = RiverRouting.AllocationRiverModel(; n),
    )
    domain = TestDomain(;
        river = TestDomainRiver(;
            network = RiverGraphs.NetworkRiver(;
                nodes_at_edge = RiverGraphs.NodesAtEdge(; src = [1, 2], dst = [2, 3]),
                edges_at_node = RiverGraphs.EdgesAtNode(;
                    src = [[], [1], [2]],
                    dst = [[1], [2], []],
                ),
            ),
            parameters = TestRiverParameters(;
                flow_width = [94.73094177246094, 94.73094177246094, 94.73094177246094],
                flow_length = [603.34375, 1059.8125, 951.96875],
            ),
        ),
        reservoir = TestDomainReservoir(;
            network = RiverGraphs.NetworkReservoir(; river_indices = [2]),
        ),
    )
    dt = RiverRouting.stable_timestep(river_flow_model, domain.river.parameters.flow_length)

    RiverRouting.update_river_channel_flow!(river_flow_model, domain.river, dt)
    ...
    RiverRouting.update_floodplain_flow!(river_flow_model, domain.river, dt)

    RiverRouting.update_bc_reservoir_model!(
    ...
    RiverRouting.update_water_depth_and_storage!(river_flow_model, domain.river, dt)

    RiverRouting.update_water_depth_and_storage!(
...
@testitem "unit: local inertial river flow with floodplain" begin
    ...
    river_flow_model = RiverRouting.RiverFlowModel(;
        routing_method = RiverRouting.LocalInertial(),
        timestepping = RiverRouting.TimeStepping(; alpha_coefficient = 0.7),
        boundary_conditions = RiverRouting.RiverFlowBC(;
            n,
            external_inflow = zeros(n),
        ...
        floodplain = RiverRouting.FloodPlainModel(;
            routing_method = RiverRouting.LocalInertial(),
            parameters = RiverRouting.FloodPlainStaggeredParameters(;
                profile = RiverRouting.FloodPlainProfile(;
            ...
            variables = RiverRouting.FloodPlainStaggeredVariables(;
                n,
                n_edges = 2,
    ...
    domain = TestDomain(;
        ...
        reservoir = TestDomainReservoir(; network = RiverGraphs.NetworkReservoir()),
    )
    dt = RiverRouting.stable_timestep(river_flow_model, domain.river.parameters.flow_length)
...
@testitem "unit: river flow on a staggered grid using Manning's equation with floodplain" begin
    ...
    n = 3
    river_flow_model = RiverRouting.RiverFlowModel(;
        routing_method = RiverRouting.ManningStaggered(),
        timestepping = RiverRouting.TimeStepping(; alpha_coefficient = 0.7),
        boundary_conditions = RiverRouting.RiverFlowBC(;
            n,
            external_inflow = zeros(n),
...
@testitem "unit: kinematic river flow including 1D floodplain schematization" begin
    ...
    river_flow_model = RiverRouting.RiverFlowModel(;
        ...
        allocation = RiverRouting.NoAllocationRiverModel(n),
        floodplain = RiverRouting.FloodPlainModel(;
            routing_method = RiverRouting.Manning(),
            parameters = RiverRouting.FloodPlainParameters(;
                profile = RiverRouting.FloodPlainProfile(;
                    depth = [0.0, 0.5, 1.0, 1.5, 2.0, 2.5],
            ...
            variables = RiverRouting.FloodPlainVariables(;
                n,
    ...
    RiverRouting.river_channel_floodplain_exchange!(river_flow_model, domain.parameters, dt)
    ...
    RiverRouting.update_floodplain_model!(river_flow_model, domain, dt)
...
@testitem "unit: update_directional_flow!" begin
    ...
    # nodes.
    n = 3
    overland_flow_model = RiverRouting.OverlandFlowModel(;
        routing_method = RiverRouting.LocalInertial(),
        timestepping = RiverRouting.TimeStepping(; alpha_coefficient = 0.7),
        boundary_conditions = RiverRouting.LocalInertialOverlandFlowBC(;
            n,
            runoff = [0.0, 0.0, 0.003001456821567986],
        ),
        parameters = RiverRouting.LocalInertialOverlandFlowParameters(;
            n,
        ...
        variables = RiverRouting.LocalInertialOverlandFlowVariables(;
            n,
    ...
    domain = TestDomain(;
        land = TestDomainLand(;
            network = RiverGraphs.NetworkLand(;
                edge_indices = RiverRouting.EdgeConnectivity(;
                    n = 3,
                    ind_x_up = [2, 3, 4],
                    ind_x_down = [4, 1, 2],
                ),
            ),
            parameters = TestLandParameters(;
    ...
    i = 2
    dt = RiverRouting.stable_timestep(overland_flow_model, domain.land.parameters)
    is_x_direction = true

    @test dt ≈ 117.10556654947368
    overland_flow_model.variables.qx0 .= overland_flow_model.variables.qx
    RiverRouting.update_directional_flow!(
        overland_flow_model,
        domain,
        i,
        dt,
...
@testitem "unit: local_inertial_update_water_depth!" begin
    ...
    river_flow_model = RiverRouting.RiverFlowModel(;
        routing_method = RiverRouting.LocalInertial(),
        timestepping = RiverRouting.TimeStepping(; alpha_coefficient = 0.7),
        boundary_conditions = RiverRouting.RiverFlowBC(; n = n_river, reservoir = nothing),
        parameters = RiverRouting.RiverFlowStaggeredParameters(;
            n = n_river,
            n_edges = 2,
            active_n = [1],
        ...
        floodplain = nothing,
        allocation = RiverRouting.NoAllocationRiverModel(n_river),
    )
    domain = TestDomain(;
        land = TestDomainLand(;
            network = RiverGraphs.NetworkLand(;
        ...
        river = TestDomainRiver(;
            network = RiverGraphs.NetworkRiver(;
                edges_at_node = RiverGraphs.EdgesAtNode(; src = [[1]], dst = [[2]]),
            ),
            parameters = TestRiverParameters(;
    ...
    storage_change = RiverRouting.compute_river_storage_change(
        overland_flow_model,
        river_flow_model,
        domain,
        3,
    ...
    @test RiverRouting.compute_external_inflow(
        river_flow_model,
        overland_flow_model,
        3,
        1,
    ...
    @test RiverRouting.compute_water_depths(
        total_storage,
        1,
        3,
        river_flow_model,
    ...
    RiverRouting.update_river_and_land_storage_and_depth!(
        overland_flow_model,
        river_flow_model,
        domain,
        3,
    ...
    @test RiverRouting.compute_land_storage_change(
        overland_flow_model,
        domain.land.network,
        2,
        dt,
    ...
    RiverRouting.update_land_storage_and_depth!(overland_flow_model, domain.land, 2, dt)
...
@testitem "local inertial long channel MacDonald (1997)" begin
    ...
    using RiverRouting: GRAVITATIONAL_ACCELERATION
    ...
    nodes_at_edge = RiverRouting.adjacent_nodes_at_edge(graph)
    n_edges = ne(graph)
    ...
    river_network = RiverGraphs.NetworkRiver(;
        nodes_at_edge = RiverGraphs.NodesAtEdge(; nodes_at_edge...),
        edges_at_node = RiverGraphs.EdgesAtNode(;
            RiverRouting.adjacent_edges_at_node(graph, nodes_at_edge)...,
        ),
    )

    params_river = TestRiverParameters(;
        flow_width = width,
        flow_length = dl,
        reservoir_outlet = zeros(n),
    )
    domain_river = TestDomainRiver(; network = river_network, parameters = params_river)
    domain = TestDomain(; river = domain_river)
    ...
    parameters = RiverRouting.RiverFlowStaggeredParameters(;
        ...
        flow_width_at_edge,
        flow_length_at_edge,
        bankfull_storage = fill(RiverRouting.MISSING_VALUE, n),
        bankfull_depth = fill(RiverRouting.MISSING_VALUE, n),
        zb,
        froude_limit,
    )

    variables = RiverRouting.RiverFlowStaggeredVariables(; n_cells = n, n_edges, h = h_init)

    boundary_conditions =
        RiverRouting.RiverFlowBC(; n, external_inflow = zeros(n), reservoir = nothing)

    sw_river = RiverRouting.RiverFlowModel(;
        routing_method = RiverRouting.LocalInertial(),
        timestepping,
        boundary_conditions,
        parameters,
    ...
    while true
        ...
        dt = RiverRouting.stable_timestep(sw_river, flow_length)
        RiverRouting.staggered_scheme_river_update!(sw_river, domain, dt, true)
...
@testitem "unit: local_inertial_flow" begin
    ...
    @test RiverRouting.local_inertial_flow(
        q_previous,
        zs0,
        zs1,
        water_depth_at_edge,
