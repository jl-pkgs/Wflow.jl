...
Group subbasins (down- to upstream), starting at the `outlet` of the basin, per
distance (1 to maximum distance `max_dist`) from the `outlet`, based on the
directed acyclic graph `g` of subbasins. 

Returns grouped subbasins `order`, ordered from `max_dist` (including subbasins
without upstream neighbor and distance < `max_dist`) to distance 0 (`outlet`).

- order[1] 汇往outlet的一级点
- order[2] 汇往上一级的二级点
...
Extract directed acyclic graph `g` representing the flow network at subbasin
level from `subbas` containing nodes with a unique id representing subbasin
outlets, `subbas_fill` with subbasin ids for the complete domain, and directed
acyclic graph `graph` representing the flow network for each subbasin cell.
"""
...
function kinwave_set_subdomains(graph, toposort, index_pit, streamorder, min_sto)
    ...
    basin_fill = fillnodata_upstream(graph, toposort, basin, 0) # 流域出水口向上游追溯
    ...
    for (i, j) in enumerate(toposort)
        index_toposort[j] = i # 记录的是原始数据的排序
    end
